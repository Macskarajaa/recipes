...
export const updateAvatar=async (uid, public_id)=>{
    ...
    try {
        ...
        console.log("Avatar módosítás/törlés hiba!");
    ...
    export const deleteAvatar = async (uid)=>{
        ...
        try {
            const docRef = doc(db,"avatars",uid)
            const docSnap=await getDoc(docRef)
            //ha nincs ilyen dokumentum, nincs teendő:
            if(!docSnap.exists()) return
            else{
                publicId=docSnap.data().public_id
                await deleteImage(publicId)//cloudinaryból törli
                await deleteDoc(docRef)//firestore:awatasból törli
            }
        } catch (error) {
